        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <MemberPicker
              ...
              description="Funcionarios titulares que harán parte activa del comité."
              employees={employees}
              selectedIds={form.principalMemberIds}
              onSetSelected={(ids) => setForm((current) => ({ ...current, principalMemberIds: ids }))}
              onToggle={(id) =>
            ...
            <MemberPicker
              ...
              description="Funcionarios de respaldo para apoyar o reemplazar miembros principales."
              employees={employees}
              selectedIds={form.alternateMemberIds}
              onSetSelected={(ids) => setForm((current) => ({ ...current, alternateMemberIds: ids }))}
              onToggle={(id) =>
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => onSetSelected(allSelected ? [] : employees.map((employee) => employee.id))}
      ...
      <div className="grid max-h-64 gap-2 overflow-y-auto rounded-lg border border-slate-200 bg-white p-2">
        {employees.length === 0 ? (
          <p className="p-3 text-sm text-slate-500">No hay funcionarios disponibles.</p>
        ) : (
          employees.map((employee) => (
            <label
              key={employee.id}
